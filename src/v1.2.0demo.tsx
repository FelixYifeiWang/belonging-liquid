...
const KinshipVisualization = () => {
  ...
  const initializeParticles = (culturesData) => {
    ...
    culturesData.forEach((culture, cultureIndex) => {
      ...
      const particleCount = Math.max(15, culture.values.length * 15);
      ...
      for (let i = 0; i < particleCount; i++) {
        const isBorder = i < borderParticleCount;
        ...
        newParticles.push({
          ...
          borderEdgeIndex: isBorder ? Math.floor((i / borderParticleCount) * culture.sides) : -1,
          borderEdgeT: isBorder ? ((i / borderParticleCount) * culture.sides) % 1 : 0,
  ...
  const randomizeCulturePositions = (focusCultureId = null) => {
    ...
    particlesRef.current = particlesRef.current.map(particle => {
      // Find the culture this particle currently belongs to
      const oldCulture = culturesDataRef.current.find(c => c.id === particle.homeCultureId);
      const newCulture = newCultures.find(c => c.id === particle.homeCultureId);
      
      if (!oldCulture || !newCulture) return particle;
      
      // Particle keeps its local position within the culture
      // but we reset its state and give it new gentle velocity
      return {
        ...particle,
        cultureId: particle.homeCultureId, // Ensure it's in the right culture
        state: 'contained',
        targetCultureId: null,
        flowPartner: null,
        activationDelay: 0,
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      culturesDataRef.current = culturesDataRef.current.map(c => {
        ...
        // Apply scope filter - hide cultures that don't match selected scope
      ...
      if (selectedScope !== 'all') {
        ...
        parentGroups.forEach((children, parentName) => {
          ...
          if (!parent) {
            ...
            // Position parent randomly in a good area of the world
            const centerX = WORLD_WIDTH / 2 + (Math.random() - 0.5) * WORLD_WIDTH * 0.6;
            const centerY = WORLD_HEIGHT / 2 + (Math.random() - 0.5) * WORLD_HEIGHT * 0.6;
      ...
      sorted.forEach(culture => {
        ...
        if (!culture.isParentGroup && (renderOpacity > 0.6 || isHovered)) {
          const fontSize = culture.layer === 3 ? 0 : 0;
      ...
      {cultures.length === 0 ? (
        ...
        <>
          <div style={{ 
            ...
            {/* Scope Filter - Hidden in focused mode */}
              ...
              <div style={{
                ...
                >
                  <div style={{ fontSize: '0.85rem', color: '#888' }}>
                    Scope Filter: <span style={{ color: '#60a5fa', textTransform: 'capitalize' }}>{selectedScope}</span>
                  </div>
                  <div style={{ 
                    fontSize: '0.75rem', 
                    color: '#888',
